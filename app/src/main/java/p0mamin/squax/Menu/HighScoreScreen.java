    ...
    public void render(float delta){
        for(int i = 0; i < num_of_el; i++){
            ...
            if(Math.abs(pos[i] + total_x) < 2) {
                ...
                font2.draw(Batch.DB.getInt("lvl"+(i+1)), pos[i] + total_x - k / 2.f * font.s,  total_y - size * 0.9f , 0.095f);
            }
        }
        finish.render(delta);
